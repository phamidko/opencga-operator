...
type Builder struct {
	...
	fcv                string
	topology           Topology
	openCGAVersion     string
	previousAC         AutomationConfig
	// OpenCGA installable versions
	versions             []OpenCGAVersionConfig
...
func (b *Builder) AddVersions(versions []OpenCGAVersionConfig) *Builder {
...
func (b *Builder) AddVersion(version OpenCGAVersionConfig) *Builder {
...
func (b *Builder) SetOpenCGAVersion(version string) *Builder {
	b.openCGAVersion = version
	return b
}
...
func (b *Builder) setFeatureCompatibilityVersionIfUpgradeIsHappening() error {
	...
	if len(b.previousAC.Processes) > 0 && b.fcv == "" {
		...
		currentVersionSemver, err := semver.Make(b.openCGAVersion)
		if err != nil {
			return errors.Errorf("current MongoDB version is not a valid semver version: %s", b.openCGAVersion)
		}
...
func (b *Builder) Build() (AutomationConfig, error) {
	...
	for i, h := range hostnames {
		...
		fcv := versions.CalculateFeatureCompatibilityVersion(b.openCGAVersion)
		if b.fcv != "" {
			fcv = b.fcv
		}
	...
	dummyConfig := buildDummyOpenCGAVersionConfig(b.openCGAVersion)
...
func versionsContain(versions []OpenCGAVersionConfig, version OpenCGAVersionConfig) bool {
	for _, v := range versions {
...
// buildDummyOpenCGAVersionConfig create a OpenCGAVersionConfig which
// will be valid for any version of MongoDB. This is used as a default if no
// versions are manually specified.
func buildDummyOpenCGAVersionConfig(version string) OpenCGAVersionConfig {
	versionConfig := OpenCGAVersionConfig{
		Name: version,
		Builds: []BuildConfig{
			{
				Platform:     "linux",
