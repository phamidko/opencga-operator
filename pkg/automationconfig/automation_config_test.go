...
func defaultOpenCGAVersion(version string) OpenCGAVersionConfig {
	return OpenCGAVersionConfig{
		Builds: []BuildConfig{
			{
				Architecture: "amd64",
...
func TestMongoDbVersions(t *testing.T) {
	ac, err := NewBuilder().
		...
		SetOpenCGAVersion("4.2.0").
		SetMembers(3).
		AddVersion(defaultOpenCGAVersion("4.2.0")).
		Build()
	...
	version2 := defaultOpenCGAVersion("4.2.3")
...
func TestModulesNotNil(t *testing.T) {
	...
	ac, err := NewBuilder().
		...
		SetOpenCGAVersion("4.2.0").
		SetMembers(3).
		AddVersion(defaultOpenCGAVersion("4.3.2")).
		Build()
...
func TestMongoDBVersionsConfig(t *testing.T) {
	...
	t.Run("Dummy Config is used when no versions are set", func(t *testing.T) {
		ac, err := NewBuilder().SetOpenCGAVersion("4.4.2").Build()
		assert.NoError(t, err)

		versions := ac.Versions
		assert.Len(t, versions, 1)
		v := versions[0]
		dummyConfig := buildDummyOpenCGAVersionConfig("4.4.2")
		assert.Equal(t, v, dummyConfig)
	})

	t.Run("Dummy Config is not used when versions are set", func(t *testing.T) {
		ac, err := NewBuilder().SetOpenCGAVersion("4.4.2").AddVersion(OpenCGAVersionConfig{
			Name: "4.4.2",
			Builds: []BuildConfig{
				{
					Platform:     "linux",
...
func createAutomationConfig(name, mongodbVersion, domain string, opts Options, auth Auth, members, acVersion int) AutomationConfig {
	ac, _ := NewBuilder().
		SetName(name).
		SetOpenCGAVersion(mongodbVersion).
		SetOptions(opts).
		SetDomain(domain).
		SetMembers(members).
		SetAuth(auth).
